...
class InventoryPage(BasePage):
    def __init__(self, page):
        super().__init__(page)
        # Locators
        self.inventory_items = self.page.locator(".inventory_item_description")
        self.inventory_items = page.locator(".inventory_item")

    # Actions
    def add_product_by_name(self, product_name):
        self.inventory_items.filter(has_text=product_name).get_by_role("button", name="Add to cart").click()

    def verify_remove_button(self, product_name, expected_text):
        button = self.inventory_items.filter(has_text=product_name).locator("button")
    ...
    def get_all_products_data(self):
        ...
        all_items = self.inventory_items.all()
